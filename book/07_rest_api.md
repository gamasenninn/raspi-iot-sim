...
## 7.4 補足: WebSocketサーバー（flask-socketio）

本書のプロジェクトには、REST APIに加えて `protocols/websocket_server.py` というWebSocketサーバーも含まれています。これはFlask-SocketIOを使い、接続中のクライアントにデバイスの状態変化をリアルタイムでプッシュ配信します。

```
REST API:     クライアント → リクエスト → サーバー → レスポンス（1回きり）
WebSocket:    クライアント ←→ サーバー（常時接続、双方向リアルタイム）
```

### REST APIとWebSocketの使い分け

| 特性 | REST API (Flask) | WebSocket (Flask-SocketIO) |
|------|-----------------|---------------------------|
| 接続方式 | リクエストごとに接続 | 常時接続 |
| データの流れ | クライアント→サーバー | 双方向 |
| リアルタイム性 | ポーリングが必要 | サーバーからプッシュ |
| 適した用途 | 状態の問い合わせ、コマンド実行 | 状態変化の即時通知 |

### WebSocketサーバーの主な機能

- **接続時**: 全デバイスの現在の状態を送信（`all_states` イベント）
- **コマンド受信**: `device_command` イベントでデバイスを操作
- **状態配信**: 1秒ごとに全デバイスの状態を全クライアントへブロードキャスト（`device_update` イベント）

REST APIと同じデバイスレジストリを共有するため、どちらのインターフェースから操作しても同じデバイスが制御されます。

> **Note:** 第10章のWebダッシュボードは、Flask-SocketIOではなくMQTT.jsを使ってMosquittoのWebSocketポート（9001）に接続する方式を採用しています。これは、Pi VM内のFlaskポート（5000）がQEMUの外に公開されていないためです。Flask-SocketIOは、Pi VM内で動作する別のPythonクライアントや、ポートフォワーディングを設定した場合に活用できます。
